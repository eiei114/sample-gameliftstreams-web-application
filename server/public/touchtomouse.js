...
/**
 * @fileoverview Utility to convert touch events to mouse events
 */

let touchEnabled = true;
let allowedTouchElement = null;

/**
 * Add event listeners which convert touch events to mouse events
 * @param {HTMLElement} element - Element to register touch events on
 * @returns {void}
 */
function RegisterTouchToMouse(element) {
    allowedTouchElement = element;
    ...
    function start(event) {
        ...
        if (!touchEnabled || (allowedTouchElement && !allowedTouchElement.contains(event.target))) {
            return;
        }
        event.preventDefault();
...
/**
 * Sets the element that is allowed to receive touch events
 * @param {HTMLElement} element - The DOM element to allow touch events on
 */
function setAllowedTouchElement(element) {
    allowedTouchElement = element;
}
